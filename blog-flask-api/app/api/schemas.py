...
from marshmallow import validate, validates, validates_schema,ValidationError, post_dump, post_load
...
class UserSchema(ma.SQLAlchemySchema):
    ...
    posts_url = ma.URLFor('api.user_all_post', values=dict(id='<id>'),dump_only=True)
    ...
    def validate_username(self, value):
        ...
        if value != old_username and User.query.filter_by(username=value).first():
    ...
    def validate_email(self, value):
        ...
        if value != old_email and User.query.filter_by(email=value).first():
...
class PostSchema(SQLAlchemySchema):
    ...
    id = ma.auto_field(dump_only=True)
    url = ma.String(dump_only=True)
    body = ma.auto_field(required=True, validate=validate.Length(
        min=1, max=280))
